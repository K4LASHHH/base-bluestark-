...
const app = new Vue({
  ...
  methods: {
    ...
    BuyCar() {
      $.post(`https://${GetParentResourceName()}/Buy`, JSON.stringify({ model: this.table.model, price: this.table.price, color: this.SColor, shoptype: this.shoptype }));
    },
    CarData(i) {
      let cars = {
        color: i.model,
      ...
      this.table.trunk = cars.trunk + 'KG';
...
window.addEventListener('message', function (event) {
  ...
  switch (v.type) {
    case "ui":
      ...
      app.table.trunk = 0 + 'KG';
      app.shoptype = v.shoptype;
      app.ui = true;
      break
    case "close":
      app.ui = false,
