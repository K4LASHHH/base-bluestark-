...
AddTextEntry(controllerMessageLoweredCar, Config.Text["RaiseInTheBed"])
AddTextEntry(controllerMessageLoweredNoCar, Config.Text["AttachVehicle"])
AddTextEntry(controllerMessageRaised, Config.Text["LowerTheBed"])
...
function drawMarker(pos)
    ...
    if dist < 25.0 then
        if KibraMekanikor() then
            DrawMarker(22, pos, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 18, 73, 211, 0.8)
            if dist < 1.5 then
                return true
            end
        end
